...
class BackupWindow:
    def __init__(self, parent, menubar: Menu):
        ...
        self.dir_view.view.grid(
            column=0, row=0, columnspan=2, rowspan=6, sticky="NSEW", padx=5, pady=3
        )

        add_dir_button = ttk.Button(
            self.mainframe, text="Add Folder", command=self.add_dir
        )
        add_dir_button.grid(column=2, row=0, sticky="NE", padx=5, pady=(5, 3))

        add_file_button = ttk.Button(
            self.mainframe, text="Add File", command=self.add_file
        )
        add_file_button.grid(column=2, row=1, sticky="NE", padx=5, pady=(5, 3))

        remove_button = ttk.Button(
            self.mainframe, text="Remove", command=self.remove_item
        )
        remove_button.grid(column=2, row=2, sticky="NE", padx=5, pady=3)

        select_button = ttk.Button(
            self.mainframe, text="Select All", command=self.select_all
        )
        select_button.grid(column=2, row=3, sticky="NE", padx=5, pady=3)

        deselect_button = ttk.Button(
            self.mainframe, text="Deselect All", command=self.deselect_all
        )
        deselect_button.grid(column=2, row=4, sticky="NE", padx=5, pady=3)

        backup_button = ttk.Button(self.mainframe, text="Backup", command=self.backup)
        backup_button.grid(column=1, row=6, sticky="NEW", padx=5, pady=(3, 5))
        ...
        self.dir_view.view.bind(
            "<<DeselectedUpdate>>", lambda _: self.save_backup_conf()
        )
        ...
        self.mainframe.after(
            500, tools.non_blocking_executor_shutdown, self.mainframe, executor, future
        )
        self.dir_view.view.after(100, self.dir_view.populate)

        # Kickstart the backup.json autoupdater
        self.mainframe.after(
            10, self.update_remote_backup_conf, datetime.datetime.now()
        )
    ...
    def load_backup_conf(self):
        ...
        self.dir_view.removed = set(removed)
    ...
    def get_local_backup_conf(self):
        if pathlib.Path("backup.json").exists():
            ...
            backup_json = open("backup_json", mode="x")
    ...
    def update_remote_backup_conf(self, last_update_time: datetime.datetime):
        ...
        self.mainframe.after(
            10, tools.non_blocking_executor_shutdown, self.mainframe, executor, future
        )
    ...
    def add_dir(self):
        ...
        dir = filedialog.askdirectory(
            parent=self.mainframe, title=f"Add {terminology}", mustexist=True
        )
    ...
    def add_file(self):
        filename = filedialog.askopenfilename(parent=self.mainframe, title="Add File")
    ...
    def __backup__(self, include_dirs: list[str], exclude_dirs: list[str]):
        ...
        rsync_progress = RsyncTracker(
            ...
            total_to_backup=total_to_backup,
        )

        rsync_progress.window.bind(
            "<<RsyncCompleted>>", lambda e: self.__end_backup__(e)
        )
    ...
    def __create_files_from_file__(self, include_dirs: list[str]):
        if platform.system() == "Windows":
            include_dirs = list(
                map(tools.win_to_rsync_readable_posix, map(pathlib.Path, include_dirs))
            )
    ...
    def __create_exclude_file__(self, exclude_dirs: list[str]):
        if platform.system() == "Windows":
            exclude_dirs = list(
                map(tools.win_to_rsync_readable_posix, map(pathlib.Path, exclude_dirs))
            )
    ...
    def total_files_to_backup(self, include_dirs: list[str], exclude_dirs: list[str]):
        ...
        total_include = sum(map(tools.how_many_files_in, total_include))

        total_exclude = map(pathlib.Path, exclude_dirs)
        total_exclude = sum(map(tools.how_many_files_in, total_exclude))
